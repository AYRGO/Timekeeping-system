...
// Helper function to clean control characters from strings for JSON encoding
if (!function_exists('cleanForJson')) {
    function cleanForJson($str) {
        if ($str === null || $str === '') return '';
        // Remove all control characters (ASCII 0-31) except newlines, then replace newlines with spaces
        $cleaned = preg_replace('/[\x00-\x08\x0B\x0C\x0E-\x1F]/', '', $str);
                ...
                <div class="flex items-center gap-2">
                    ...
                    if ($type === 'Time') {
                        ...
                        <button onclick="showActivityDetails('Time Adjustment', `<?= htmlspecialchars(json_encode([
                            ...
                            'reason' => cleanForJson($activity['reason'] ?? ''),
                            'explanation' => cleanForJson($activity['explanation'] ?? ''),
                    ...
                    <?php } elseif ($type === 'Overtime') { ?>
                        <button onclick="showActivityDetails('Overtime Request', `<?= htmlspecialchars(json_encode([
                            ...
                            'reason' => cleanForJson($activity['ot_reason'] ?? ''),
                    ...
                    <?php } else { ?>
                        ...
                        ?>
                        <button onclick="showActivityDetails('<?= $type ?> Request', `<?= htmlspecialchars(json_encode($fallbackPayload, JSON_HEX_TAG | JSON_HEX_AMP | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_UNESCAPED_UNICODE)) ?>`)" 
                                class="inline-flex px-3 py-1.5 text-xs font-semibold text-blue-700 bg-blue-100 hover:bg-blue-200 rounded-md transition-all duration-200 shadow-sm ring-1 ring-blue-200/50">
                            View Details
                        </button>
                    <?php } ?>
